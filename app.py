...
def add_flat():
    ...
    if request.method == "POST":
        ...
        cur.execute("""
        INSERT INTO flats
        (building_id, user_id, floor_no, flat_no, bhk, area_sqft, rent_amount, security_deposit)
...
def add_tenant():
    ...
    if request.method == "POST":
        ...
        cur.execute("""
    INSERT INTO tenants 
    (flat_id, user_id, full_name, mobile, profession, is_family, move_in_date, paid_months)
    VALUES (%s, %s, %s, %s, %s, %s, CURRENT_DATE, 0)
""", (
    flat_id,
    session["user_id"],
    full_name,
...
def tenant_profile(tenant_id):
    ...
    cur.execute("""
        SELECT 
            ...
            (
                DATE_PART('year', AGE(CURRENT_DATE, t.move_in_date)) * 12 +
                DATE_PART('month', AGE(CURRENT_DATE, t.move_in_date))
            ) AS total_months,
...
@app.route("/add-rent-payment/<int:tenant_id>", methods=["GET", "POST"])
def add_rent_payment(tenant_id):
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    if request.method == "POST":
        ...
        # ✅ 1. Payment Entry Save
        cur.execute("""
            INSERT INTO rent_payments (tenant_id, amount_paid, month_for, notes)
            VALUES (%s, %s, %s, %s)
        """, (tenant_id, amount_paid, month_for, notes))

        # ✅ 2. Paid Months Auto Increase (Only Add)
        cur.execute("""
            UPDATE tenants
            SET paid_months = COALESCE(paid_months, 0) + 1
            WHERE id = %s
        """, (tenant_id,))
    ...
    return render_template("add_rent_payment.html", tenant_id=tenant_id)


@app.route("/pending-rent")
def pending_rent():
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    return render_template("pending_rent.html", tenants=tenants)

@app.route("/rent-history/<int:tenant_id>")
def rent_history(tenant_id):
    conn = get_db_connection()
    cur = conn.cursor()

    # Tenant Name
    cur.execute("SELECT full_name FROM tenants WHERE id=%s", (tenant_id,))
    tenant = cur.fetchone()

    # Payment History
    cur.execute("""
        SELECT amount_paid, month_for, notes, payment_date
        FROM rent_payments
        WHERE tenant_id=%s
...
def signup():
    if request.method == "POST":
        ...
        cur.execute("""
        INSERT INTO users (full_name, mobile, email, password_hash, role)
        VALUES (%s, %s, %s, %s, 'OWNER')
""",   (full_name, mobile, email, hashed))
